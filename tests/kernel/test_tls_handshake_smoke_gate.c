...
static void fail(const char *msg) {
  printf("[tls-handshake-smoke] FAIL: %s\n", msg);
  g_failures++;
}

static void test_state_reset_zeros_fields(void) {
  struct tls_handshake_smoke_state state;
  memset(&state, 0xAA, sizeof(state));
  tls_handshake_smoke_state_reset(&state);
...
static void test_state_reset_null_safe(void) {
  tls_handshake_smoke_state_reset(NULL);
}

static void test_gate_predicate(void) {
  if (tls_handshake_smoke_gate_observed(0u)) {
    fail("zero successes must not satisfy gate");
  }
  if (!tls_handshake_smoke_gate_observed(1u)) {
    fail("one success must satisfy gate");
  }
  if (!tls_handshake_smoke_gate_observed(5u)) {
...
static void test_nonzero_exit_ignored(void) {
  struct tls_handshake_smoke_state state;
  tls_handshake_smoke_state_reset(&state);
  /* Any non-zero exit (failure of the valid GET or the bad-cert refusal,
   * or a fault-killed >= 128 exit) must not flip the gate. */
  if (tls_handshake_smoke_observe_exit(&state, 1) != 0 ||
      tls_handshake_smoke_observe_exit(&state, 142) != 0 ||
      tls_handshake_smoke_observe_exit(&state, -1) != 0) {
...
static void test_success_exit_latches_once(void) {
  struct tls_handshake_smoke_state state;
  tls_handshake_smoke_state_reset(&state);
  if (tls_handshake_smoke_observe_exit(
          &state, TLS_HANDSHAKE_SMOKE_SUCCESS_EXIT_CODE) != 1) {
...
static void test_global_latch_single_emission(void) {
  ...
  tls_handshake_smoke_global_reset();
  if (tls_handshake_smoke_try_latch_exit_global(0) != 1) {
    fail("global latch must fire on first success exit");
  }
  if (tls_handshake_smoke_try_latch_exit_global(0) != 0) {
...
int run_tls_handshake_smoke_gate_tests(void) {
  ...
  if (g_failures == 0) {
    printf("[tls-handshake-smoke] all gate tests passed\n");
  }
  return g_failures;
}
