 ...
 *
 * Foundational crypto vector coverage (SHA-256 RFC 6234, PBKDF2-SHA256
 * RFC 6070, AES-XTS NIST SP 800-38E roundtrip + raw-cipher hashes
 * pinned to alpha.220), block-device wrapper chain (chunk + offset)
 * encryption roundtrip, constant-time comparator semantics and
 * `sha256_clear` zeroing contract. Owns the entry point
 * `run_crypt_vector_tests` which runs the local cases then dispatches
 * to the companion files:
 *
 *   - `tests/test_crypt_vectors_aead.c`: ed25519 + HKDF + ChaCha20 +
 *     Poly1305 + ChaCha20-Poly1305 AEAD.
 *   - `tests/test_crypt_vectors_kdf.c`:  X25519 + BLAKE2b + Argon2id +
 *     `crypt_derive_xts_keys_argon2id`.
 *
 * Shared hex helpers (`hex_nibble`, `decode_hex`, `expect_hex`) are
 * defined non-static here and declared via
 * `tests/test_crypt_vectors_internal.h`. The mem-block-device
 * harnesses (`mem_block_device` + `mem_lba_device`) stay local because
 * only the AES-XTS roundtrip cases use them.
 *
 * Carved out of the historical single-file
 * `tests/test_crypt_vectors.c` (1910 LOC) at the 2026-05-15 monolith
 * refactor so each host-test translation unit stays under the 900-line
 * layout limit.
 */
#include <stdio.h>
#include <string.h>
...
int test_crypt_vectors_decode_hex(const char *hex, uint8_t *out,
                                  size_t out_len) {
  size_t i = 0u;
  if (!hex || !out) {
    return -1;
  ...
  for (; i < out_len; ++i) {
    uint8_t hi = test_crypt_vectors_hex_nibble(hex[i * 2u]);
    uint8_t lo = test_crypt_vectors_hex_nibble(hex[i * 2u + 1u]);
...
int test_crypt_vectors_expect_hex(const char *name, const uint8_t *actual,
  ...
  /* Buffer sized to fit the largest cryptographic vector tested across
   * all three crypt_vectors translation units (HKDF-SHA256 RFC 5869
   * Test Case 2 produces an 82-byte OKM; 256 leaves headroom for
   * future vectors). */
  uint8_t expected[256];
  if (!actual || !expected_hex || len > sizeof(expected) ||
      test_crypt_vectors_decode_hex(expected_hex, expected, len) != 0) {
...
int run_crypt_vector_tests(void) {
  ...
  fails += test_crypt_vectors_aead_cases();
  fails += test_crypt_vectors_kdf_cases();
  if (fails == 0) {
