...
#include "tools/grub_cfg_builder.h"

static int build_iso_cfg(const char *out_path) {
    const struct grub_menu_entry entries[] = {
        { .title = "Noir Guided Installation System", .multiboot_path = "/boot/installer.bin" },
        { .title = "NoirOS (boot via ISO + disco instalado)", .multiboot_path = "/boot/noiros.bin" },
    };
    ...
    return grub_cfg_write_file(out_path, &opts, entries, sizeof(entries) / sizeof(entries[0]));
}
