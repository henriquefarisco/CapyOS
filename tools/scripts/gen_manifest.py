#!/usr/bin/env python3
"""
Gera manifest.bin para NoirOS (boot_manifest).

Layout:
  entry 0: NORMAL kernel (obrigatório)
  entry 1: RECOVERY kernel (opcional)

LBA/sectores são relativos à partição BOOT (offset aplicado no loader).
"""
import argparse
import math
...
def main():
    ap = argparse.ArgumentParser(description="Gera manifest.bin (boot_manifest) para NoirOS.")
    ap.add_argument("--out", default="build/manifest.bin", type=Path, help="Arquivo de saída manifest.bin")
    ap.add_argument("--kernel", required=True, type=Path, help="Caminho do kernel principal (ELF)")
    ap.add_argument("--recovery", type=Path, help="Kernel de recuperação (opcional)")
    ap.add_argument("--kernel-lba", type=int, default=1, help="LBA (relativo à partição BOOT) do kernel principal (default: 1)")
    ap.add_argument("--recovery-lba", type=int, help="LBA (relativo à BOOT) do kernel de recuperação")
    args = ap.parse_args()
