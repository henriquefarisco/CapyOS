#!/usr/bin/env python3
"""
CapyOS x64 smoke for end-to-end ring-3 preemption (M4 phase 8f.3).

Boots a kernel built with `-DCAPYOS_PREEMPTIVE_SCHEDULER
-DCAPYOS_BOOT_RUN_HELLO` AND a userland hello rebuilt with
`-DCAPYOS_HELLO_BUSY` (so the embedded hello loops forever emitting
markers via SYS_WRITE). The smoke verifies the entire phase 8f
stack works end-to-end:

  - 8f.1 TSS scaffolding: the FIRST APIC tick fired from ring 3
    does NOT triple-fault the kernel (without a TSS the CPU has no
    RSP0 to push the IRET frame onto, and ring-3 -> ring-0
    transitions raise #DF immediately).
  - 8f.2 per-task RSP0 swap: subsequent ticks land on hello's own
    per-task kernel stack instead of clobbering the shared syscall
    stack.
  - context_switch + iretq round-trip: iretq correctly returns to
    ring 3 after every tick so hello_busy continues looping.

Pass criteria (matched against the kernel debug-console log file):

  * "[user_init] CAPYOS_BOOT_RUN_HELLO defined;" present.
  * "[busyU]" appears AT LEAST `BUSY_MIN` times. The minimum count
    is the asymmetric check that turns "the marker fired once and
    then the kernel froze" into a smoke failure - any kernel-side
    bug that misses the iretq-back-to-ring-3 path would emit a
    single marker (or none) before silently hanging.
  * "panic" absent.
  * "[user_init] hello spawn returned without entering Ring 3."
    absent (regression guard for spawn / ELF failures).
...
# How many [busyU] occurrences we require. The number is intentionally
# small so the smoke completes within a reasonable QEMU-TCG timeout but
# still proves multiple iretq round-trips through ring 3.
BUSY_MIN = 3

SUCCESS_MARKERS = (
    "[user_init] CAPYOS_BOOT_RUN_HELLO defined;",
    # The first [busyU0] proves enter_user_mode dropped to ring 3 and
    # the syscall path round-tripped at least once. The BUSY_MIN
    # post-condition below proves the loop kept making progress
    # despite (or because of) APIC ticks firing from ring 3.
    # Phase 8f.5 added a per-task `rank` channel: the single-task
    # build (this smoke) sees rank=0 and emits [busyU0]; the
    # two-task variant in smoke-x64-preemptive-user-2task asserts
    # both [busyU0] AND [busyU1] appear.
    "[busyU0]",
)
FAILURE_MARKERS = (
    "panic",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--timeout", type=float, default=45.0,
                        help="Seconds to wait for BUSY_MIN markers")
    parser.add_argument("--busy-min", type=int, default=BUSY_MIN,
                        help="Minimum [busyU] count for success")
    parser.add_argument(
        "--log",
        default="build/ci/smoke_x64_preemptive_user.log",
        help="QEMU stdout/stderr log path")
    parser.add_argument(
        "--debugcon-log",
        default="build/ci/smoke_x64_preemptive_user.debugcon.log",
        help="Kernel debug-console log path (port 0xE9)")
    parser.add_argument(
        "--disk",
        default="build/ci/smoke_x64_preemptive_user.img",
...
def main() -> int:
    ...
    for required in (bootx64, kernel, manifest):
        if not required.exists():
            print(f"[err] missing build artifact: {required}\n"
                  ...
                  f"-DCAPYOS_BOOT_RUN_HELLO' EXTRA_USERLAND_CFLAGS="
                  f"'-DCAPYOS_HELLO_BUSY'?", file=sys.stderr)
            return 2
    ...
    busy = busy_count(text)

    if success:
        print(f"[ok] preemptive-user smoke passed in <={args.timeout:.0f}s")
        for m in found:
            print(f"     + {m!r} present")
        print(f"     + [busyU] count = {busy} (>= {args.busy_min})")
        rc = 0
    else:
        print("[err] preemptive-user smoke failed", file=sys.stderr)
        if failure_reason is not None:
            print(f"      reason: failure marker {failure_reason!r} "
                  f"appeared", file=sys.stderr)
        for m in missing:
            print(f"      - missing: {m!r}", file=sys.stderr)
        if busy < args.busy_min:
            print(f"      [busyU] count = {busy} (< {args.busy_min}) -- "
                  f"either the user task never reached ring 3 or the "
                  f"kernel hung after the first tick", file=sys.stderr)
