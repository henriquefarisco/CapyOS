# CapyOS 0.8.0-alpha.183+20260512
...
loginwindow e adiciona o `retention_plan`, uma camada pura sobre o `archive_plan`.

O incremento prepara somente tickets declarativos de retention para futura
integração GUI/compositor/display/auditoria persistente. Ele não persiste
retention, não persiste archive, não persiste journal, não persiste ledger, não
persiste recibo, não persiste registro, não anexa log, não escreve estado real,
não executa sincronização CPU/GPU, não submete display/output e não habilita
autenticação gráfica.

## Entregue

- Adicionado `LOGIN_WINDOW_CREDENTIAL_SCREEN_RETENTION_PLAN_VERSION`.
- Adicionado `struct login_window_credential_screen_retention_plan`.
- Adicionada `login_window_credential_screen_retention_plan_build()` para
  selecionar tickets `credential-screen-retention-ticket`,
  `text-recovery-retention-ticket`, `text-login-resume-retention-ticket` ou
  `text-login-fallback-retention-ticket`.
- O builder é uma camada pura sobre `login_window_credential_screen_archive_plan`.
- Adicionados testes planejados fail-closed em `tests/test_login_runtime.c` para:
  ...
  - archive plan ausente;
  - archive plan inseguro;
  - archive plan adulterado com estados já submetidos.

## Garantias de segurança

O `retention_plan` só marca `retention_plan_safe=1` quando o `archive_plan` de
entrada está seguro e preserva todos os invariantes declarativos anteriores:

- archive exigido e permitido, mas não submetido;
- archive ticket/target selecionados;
- persistência de archive desabilitada;
- sync CPU/GPU de archive desabilitado;
...
integração real com retenção persistente, display ou autenticação gráfica.
