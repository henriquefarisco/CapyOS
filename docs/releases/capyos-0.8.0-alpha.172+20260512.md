# CapyOS 0.8.0-alpha.172+20260512
...
completion plan seguro para a tela de credenciais do loginwindow. O completion
plan consome o deadline plan seguro e publica somente um ticket declarativo de
completion para futura integracao GUI/compositor/display, sem mapear memoria
real, sem escrever pixels, sem executar flush/cache/barreira, sem estabelecer
...
sem expirar deadline, sem reportar completion real, sem acknowledge de
...
- Adicionado `LOGIN_WINDOW_CREDENTIAL_SCREEN_COMPLETION_PLAN_VERSION`.
- Adicionado `struct login_window_credential_screen_completion_plan`.
- Adicionada `login_window_credential_screen_completion_plan_build()` para
  selecionar tickets `credential-screen-completion-ticket`,
  `text-recovery-completion-ticket`, `text-login-resume-completion-ticket` ou
  `text-login-fallback-completion-ticket`.
- Testes planejados cobrem completion declarativo de credencial, recuperacao
  textual, resume textual, submit bloqueado, acao desconhecida, deadline plan
  ausente, deadline plan inseguro e tentativa de propagar estado unsafe ja
  armado/reportado.
...
- O completion plan exige deadline plan seguro, deadline permitido mas nao
  ...
  deadline nao reportado, CPU/GPU sync desabilitado, sync nao submetido,
  wait/signal/deadline/completion de sync nao executados, timeline nao submetida,
  wait/signal/semaphore de timeline desabilitados, valor de timeline nao alocado
  nem publicado, fence/barrier/flush/framebuffer/blit/output/display/scanout/
...
- Submit grafico nunca vira autenticacao: converge para
  `text-login-fallback-completion-ticket` com `gui-submit-disabled`.
...
- `completion_allowed`, `completion_ticket_selected` e
  `completion_target_selected` preparam escalabilidade futura sem reportar
  completion real, acknowledge, CPU/GPU sync, output, compositor ou flip real
  neste patch.
