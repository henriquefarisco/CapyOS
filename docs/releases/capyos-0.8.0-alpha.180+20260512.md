# CapyOS 0.8.0-alpha.180+20260512
...
loginwindow e adiciona o `ledger_plan`, uma camada pura sobre o `receipt_plan`.

O incremento prepara somente tickets declarativos de ledger para futura
integração GUI/compositor/display/auditoria persistente. Ele não persiste
ledger, não persiste recibo, não persiste registro, não anexa log, não escreve
estado real, não executa sincronização CPU/GPU, não submete display/output e não
habilita autenticação gráfica.

## Entregue

- Adicionado `LOGIN_WINDOW_CREDENTIAL_SCREEN_LEDGER_PLAN_VERSION`.
- Adicionado `struct login_window_credential_screen_ledger_plan`.
- Adicionada `login_window_credential_screen_ledger_plan_build()` para selecionar
  tickets `credential-screen-ledger-ticket`, `text-recovery-ledger-ticket`,
  `text-login-resume-ledger-ticket` ou `text-login-fallback-ledger-ticket`.
- O builder é uma camada pura sobre `login_window_credential_screen_receipt_plan`.
- Corrigida a propagação segura de `record_error=0` no estado seguro do
  `receipt_plan`, mantendo o novo predicado de ledger estrito sem afrouxar
  invariantes.
- Adicionados testes planejados fail-closed em `tests/test_login_runtime.c` para:
  ...
  - receipt plan ausente;
  - receipt plan inseguro;
  - receipt plan adulterado com estados já submetidos.

## Garantias de segurança

O `ledger_plan` só marca `ledger_plan_safe=1` quando o `receipt_plan` de entrada
está seguro e preserva todos os invariantes declarativos anteriores:

- receipt exigido e permitido, mas não submetido;
- receipt ticket/target selecionados;
- persistência de receipt desabilitada;
- sync CPU/GPU de receipt desabilitado;
...
integração real com persistência, display ou autenticação gráfica.
