...
make release-ci-smoke-evidence RELEASE_TAG=0.8.0-alpha.93+20260510 SMOKE_X64_VMWARE_ARGS="$SMOKE_X64_VMWARE_ARGS"
```

Para conferir um manifesto de evidência já publicado:

```bash
make verify-release-ci-smoke-evidence RELEASE_TAG=0.8.0-alpha.93+20260510 SMOKE_X64_VMWARE_ARGS="$SMOKE_X64_VMWARE_ARGS"
```
...
- Exige que serial e summary contenham todos os markers obrigatórios.
...
- Rejeita evidências contendo markers graves de falha, como panic ou triple fault.
