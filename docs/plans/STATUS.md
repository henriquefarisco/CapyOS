...
**Data:** 2026-05-14 · **Versão:** `0.8.0-alpha.237+20260514` · **Plataforma:** VMware + UEFI + E1000
...
- **Plano sequencial novo:** Etapa 2 tecnicamente fechada por código/docs; 1/15 etapas oficialmente fechadas até validação externa.
- **Etapa em validação:** Etapa 2 — Sessão gráfica operacional, 100% entregue por escopo técnico.
- **Restante da Etapa 2:** execução externa dos gates finais em VMware + UEFI + E1000.
- **Etapas bloqueadas:** Etapas 3-15 dependem da validação externa integral da etapa anterior.

## Visão executiva da Etapa 2

`alpha.237` entrega o gate externo final `mouse-events` e completa o escopo
técnico da Etapa 2. A etapa só fica operacionalmente aceita quando os gates
abaixo forem executados fora desta máquina na plataforma oficial:

| Bloco restante | Peso | Slice alvo | Saída exigida |
|---|---:|---|---|
| Validação externa final | aceite operacional | `alpha.237` | CI/operador executa `make test`, `make layout-audit`, `make all64`, `make smoke-x64-vmware-mouse-events` e gates de readiness/evidência/aceitação/promoção com evidências anexadas. |

Critério de aceite operacional: a Etapa 2 só libera a Etapa 3 após `make test`,
`make layout-audit`, `make all64`, `make release-check` e o smoke
`mouse-events` oficial serem executados fora desta máquina com evidência anexada.
Novas falhas encontradas durante esses gates entram como bugs dentro do bloco
acima, não como novos entregáveis de roadmap, salvo mudança explícita deste plano.
...
> entregues por contrato até `alpha.237`; resta execução externa dos gates.
>
...
>
> **Pendente externo:** execução fora desta máquina dos gates finais antes de
> liberar a Etapa 3.

> **Nota sobre a tabela abaixo:** a linha longa da Etapa 2 preserva histórico
> detalhado acumulado; o status executivo autoritativo atual é validação externa
> de `alpha.237`, conforme os itens acima.
...
| 3 | CapyDisplay 2D | Bloqueada | depende da Etapa 2 |
| 4 | Driver framework + VM drivers | Bloqueada | depende da Etapa 3 |
| 5 | TLS userland real | Bloqueada | depende da Etapa 4 |
| 6 | Release/update gate oficial | Bloqueada | depende da Etapa 5 |
| 7 | Apps básicos do desktop | Bloqueada | depende da Etapa 6 |
| 8 | Package manager + SDK + ABI estável | Bloqueada | depende da Etapa 7 |
| 9 | CapyLX L0-L2: CLI estático | Bloqueada | depende da Etapa 8 |
| 10 | CapyLX L3-L5: POSIX amplo | Bloqueada | depende da Etapa 9 |
| 11 | Wayland bridge | Bloqueada | depende da Etapa 10 |
| 12 | Mesa/Vulkan path | Bloqueada | depende da Etapa 11 |
| 13 | JS engine sandboxed | Bloqueada | depende da Etapa 12 |
| 14 | CapyLang | Bloqueada | depende da Etapa 13 |
| 15 | Plataforma 1.0 hardening | Bloqueada | depende da Etapa 14 |
...
Status atual: escopo técnico entregue em `alpha.237`; resta execução externa dos gates oficiais antes de liberar a Etapa 3.
...
Entregáveis concluídos e pendência externa:
...
- [ ] Execução externa final dos gates.
...
A Etapa 2 está tecnicamente fechada em `alpha.237` com gates externos `gui-session` e `mouse-events` entregues por contrato, markers seriais e wiring de release. Resta a execução externa final dos gates antes de liberar a Etapa 3.
Etapas 3-15 continuam bloqueadas até o aceite operacional externo da Etapa 2.
