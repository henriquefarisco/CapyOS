...
**Data:** 2026-05-15 · **Versão:** `0.8.0-alpha.237+20260514` · **Plataforma:** VMware + UEFI + E1000 · **Público alvo:** usuário desktop comum
...
- **Plano sequencial novo (pós-reordenação ROI):** Etapa 2 tecnicamente fechada por código/docs; 1/16 etapas oficialmente fechadas até validação externa.
...
- **Próxima etapa após Etapa 2:** Etapa 3 reorganizada — Driver framework + entrada USB HID + storage estável (era Etapa 4 antes da reordenação).
- **Etapas bloqueadas:** Etapas 3-16 dependem da validação externa integral da etapa anterior.
...
|---|---:|---|---|
| Validação externa final | aceite operacional | `alpha.237` | CI/operador executa `make test`, `make layout-audit`, `make all64`, `make release-check`, `make smoke-x64-vmware-mouse-events` e gates de readiness/evidência/aceitação/promoção com evidências anexadas. |
...
> **Nota sobre a tabela abaixo:** após a reordenação por ROI em 2026-05-15, a numeração das Etapas 3-16 mudou. A linha longa da Etapa 2 preserva o histórico técnico acumulado; o status executivo autoritativo atual é validação externa de `alpha.237`. A sequência antiga está em [`historical/capyos-master-plan-pre-roi-reorder.md`](historical/capyos-master-plan-pre-roi-reorder.md).
...
| 3 | Driver framework + entrada USB HID + storage estável | Bloqueada | depende da Etapa 2 |
...
Etapas 3-16 (sequência reorganizada por ROI em 2026-05-15) continuam bloqueadas até o aceite operacional externo da Etapa 2. A próxima etapa após a Etapa 2 é **Etapa 3 — Driver framework + entrada USB HID + storage estável**.
