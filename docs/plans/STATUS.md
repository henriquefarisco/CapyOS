...
**Data:** 2026-07-27 | **Versao:** `0.8.0-alpha.319+20260728` | **Plataforma oficial:** VMware + UEFI + E1000 | **Publico alvo:** usuario desktop comum
...
- **Trabalho in-tree `alpha.319` — gate do ciclo A/B assinado entregue; Etapa 8
  SEGUE ABERTA e a release NÃO deve ser taggeada:** o gate que produz a evidência
  do último critério existe e seus gates host estão verdes (`make test`,
  `layout-audit`, `version-audit`, `update-ab-selftest`, `all64`, `iso-uefi`), mas
  a **primeira execução real do gate expôs uma regressão de boot do kernel**: o
  loader completa `ExitBootServices` e entrega o controle (último marker debugcon
  `J`), e o kernel morre com `#UD - Invalid Opcode` antes do primeiro
  `dbgcon_putc('H')` de `kernel_main64`, com `RIP` dentro de `.rodata`
  (`.rodata + 0x1B7`). Não é efeito do gate nem da âncora de laboratório:
  `make smoke-x64-cli` com a build oficial falha idêntico. Coerente com o registro
  do `alpha.318` ("não reexecutados: gates de instalador QEMU/VMware"), que tornou
  o loader A/B autoritativo sem nunca validar um boot de kernel em runtime; o
  `alpha.317` foi a última release com QEMU/VMware verdes. Ação bloqueante:
  bissectar `alpha.317` → `alpha.318` com `make smoke-x64-cli`, resolver o
  endereço com `objdump`/`nm` na build exata, corrigir e então rodar o gate A/B.
...
- **Plano sequencial novo (pós-reordenação ROI):** Etapas 1-7 oficialmente fechadas; 7/16 etapas concluídas; Etapa 8 em andamento com um critério aberto, hoje bloqueado pela regressão de boot descrita acima.
...
## Repositórios apartados (estado em alpha.319)
...
[`compatibility-audit-2026-07-27-alpha318.md`](../reference/integration/compatibility-audit-2026-07-27-alpha318.md).

| Repo apartado | Versão atual | Owner autoritativo | Gate de integração CapyOS |
|---|---|---|---|
| [`CapyUI`](../../../CapyUI) | `2.24.1` | widget 2.22/schema 7, desktop-session v1 e hardening de logout/worker/session scoped | Etapas 4, 6, 7 e regressao da Etapa 8 |
...
| 8 | Release/update gate oficial + instalador polido | **Em andamento** | updater A/B assinado e lifecycle pós-reboot ainda abertos |
