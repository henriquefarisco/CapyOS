...
- **Etapa 7 entregue em `alpha.312` para o escopo web estatico:** browser
  grafico ring-3 com toolbar, navegacao, historico, links, CSS, imagens,
  redirects, cache e limites; smoke QEMU ponta a ponta aprovado. JavaScript e
  WebAssembly continuam fora do escopo. O hardening de lifecycle elimina o
...
- **Etapa 8 concluida na `0.9.2`:** staging, apply no slot inativo, tentativa
  unica, confirmacao de saude e rollback foram comprovados no track oficial. A
  release foi promovida como Latest imutavel; os materiais Ed25519, os nove
  modulos e as rotas publicas foram validados. O run VMware de producao
...
- **Release stable `0.9.2+20260826`:** preserva ABI e pins dos repositorios
  ...
  O bootstrap da `0.9.1` usou perfil estatico derivado do VMnet8, sem reivindicar
  DHCP no predecessor; E1000/DHCP nativos da `0.9.2` passaram separadamente no
  gate VMware lab. A Etapa 9 esta desbloqueada e e a proxima.
...
- **Etapa 8 — hardening do instalador:** o loader deixa de escolher o maior
  ...
  exige 1 GiB minimo para DATA, numera somente alvos elegiveis, mostra `PathId`
  e capacidade, exige selecao explicita + token `ERASE` e revalida a mesma midia
  e o mesmo plano antes de `wipe_blocks()`. O smoke ISO responde ao mesmo fluxo e
  falha se nao houver exatamente um alvo elegivel. A fatia eliminou o wipe antes
  do preflight; o criterio do instalador permaneceu aberto até o gate VMware
  multi-disco aprovado em `alpha.316` provar fresh install, reboot, login,
  persistencia e disco guard intacto.

- **Etapa 8 — regressão first boot/desktop corrigida:** objetos gerados por
  smokes com `EXTRA_CFLAGS64` podiam ser reutilizados por um `make iso-uefi`
  normal. Em particular, `CAPYOS_CAPYAI_GUI_ASYNC_SMOKE` chama
  `desktop_stop()` deliberadamente, fazendo o desktop piscar e voltar ao shell
  em toda inicialização. O build agora pina uma assinatura da variante
  (toolchain/profile/CFLAGS/siblings), invalida kernel+userland ao mudar e
  permite reuse apenas dentro do target de smoke. A ISO oficial também rejeita
  CAPYCFG com setup/senha preseedados e kernel com marker do smoke. No first
  boot, `first-run.done` passa a ser autoritativo e só é gravado depois da
  seleção de módulos; users+config sem marker não pulam mais o wizard. O smoke
  usa senha não-default e falha se disco vazio chegar direto ao login.

- **Release coordenada `alpha.315`:** consolida as três fatias da Etapa 8 acima,
  integra CapyAI `0.2.0` e CapyUI `2.24.1` sem bump de ABI, impede workers de
  herdar principal autenticado, vincula snapshots sanitizados em dispatch
  scoped e atualiza os GitHub Actions pinados. O updater permanece fail-closed
  para apply (`-60`) e o instalador multi-disco VMware continua como gate
  externo aberto; esta release não encerra a Etapa 8.

- **Trabalho in-tree pós-`alpha.315` — evidência do instalador:** corrigido o
  falso positivo em que login gráfico pulava a escrita e releitura do marker de
  persistência. O harness agora retorna ao CLI, lida com reboot intermediário do
  setup e só aprova após novo login e releitura. O preflight
  `smoke-x64-qemu-installer-wizard` usa alvo de 2 GiB + guard determinístico maior,
  seleciona pela capacidade/`PathId`, exige alvo alterado e guard com SHA-256
  integral idêntico. O gate QEMU e o full install networked passaram no host em
  2026-07-17, incluindo retorno desktop→TTY pelo sentinel real, módulo concluído
  com reboot e marker relido. Continua feedback de desenvolvimento; VMware
  provisionada, CI oficial e todos os quatro critérios da Etapa 8 permanecem
  abertos.

- **Release coordenada `alpha.316`:** publica o harness fail-closed do
  instalador com seleção multi-disco por capacidade/`PathId`, preservação do
  disco guard, logs sem recovery key e contrato VMware UEFI/E1000. Integra o
  CapyAI `0.2.1`, cujo gate massivo exige 100% de comando/risco nos casos
  gerados, zero underclassification e zero leakage entre splits. O índice de
  módulos fica pinado nesta release e é montado apenas com assets publicados e
  compatíveis. O gate VMware UEFI/E1000 multi-disco passou em 2026-07-20 com
  alvo alterado, guard byte-idêntico e marker relido após reboot. A Etapa 8
  continua aberta para update A/B assinado e gates externos restantes; esta
  release fecha a fatia do instalador.

- **Release `alpha.317` — fundação A/B fail-closed + handoff v9:** duas
  imagens abstratas de 512 bytes usam CRC, geração monotônica, alternância,
  flush e readback integral. O planner fixa A/B dentro dos 524.288 setores do
  BOOT sem mudar GPT: A começa em manifest/payload 0/1, B no midpoint e os dois
  controles ocupam os setores finais. O metadata liga cada slot a LBA,
  capacidade, tamanho, versão ASCII e SHA-256; reopen valida uma geometria
  confiável fornecida pelo provider. `confirmed/pending/tries` são explícitos,
  uma tentativa é consumida antes do boot simulado, health exige slot+geração e
  tentativa esgotada restaura o confirmed antes da seleção. Split-brain,
  pending com mirror degradado e write/flush/readback incertos falham fechados.
  A camada host-only `boot_slot_store` escreve payload-first apenas sobre slot
  inativo `EMPTY`/`FAILED`, recusa restage de `VALID` antes de qualquer I/O, faz
  flush, readback SHA-256 + padding integral e grava/valida o header por último;
  não muta o confirmed manager. O `boot_slot_block_provider` valida headers,
  CRCs e igualdade byte a byte dos arrays GPT primary+backup, layout canônico,
  GUIDs/atributos e binding exato antes de mapear o store ao BOOT raw. O handoff
  v9 append-only leva ranges+GUIDs de ESP/BOOT/DATA com flags canônicas fechadas;
  UEFI vincula raw/ESP/DATA pelo mesmo prefixo físico de Device Path, centraliza
  I/O destrutivo com `IoAlign`, propaga falhas de `FlushBlocks` antes de anunciar
  sucesso/reboot e valida o manifest BOOT sem type-punning. O runtime nativo exige
  exatamente um disco correspondente e revalida o fingerprint antes do mount. Discos
  antigos com GUIDs duplicados usam modo legado validado apenas para mount,
  nunca para update. Como flush durável e autorização de staging vinculada à
  geração do manager ainda não existem na capability de produção, qualquer
  provider de saída é integralmente zerado; apply, health e rollback continuam
  `-60`, e nenhum critério adicional/de update é encerrado. `release-check` ELF,
  checksums, QEMU e VMware UEFI/E1000 multi-disco passaram na ISO final
  (`98ef6215...e053`), com target alterado, guard byte-idêntico, first boot,
  login e marker persistente relido. O índice local tem nove entradas; o gate
  networked segue bloqueado pelo asset alpha.317 ainda não publicado (HTTP 404).

- **Release `alpha.318` — apply A/B persistente habilitado:** o flush durável já
  existe nos três backends nativos (NVMe `FLUSH`; AHCI/ATA `FLUSH CACHE`/`EXT`
  descoberto por IDENTIFY word 83) e é pré-requisito do provider, então a
  capability deixa de ser zerada e passa a ser negada apenas com motivo
  declarado. O handoff chega a v10 com token de tentativa de 16 bytes
  (`flags`/`slot`/`generation`, combinações canônicas fechadas) sobre o prefixo
  v9 congelado de 560 bytes; o loader é autoritativamente A/B, consome a única
  tentativa com commit durável, valida header/payload/SHA-256/padding antes de
  carregar e tenta o rollback quando o slot pendente não carrega. `kernel_main`
  valida o token pelo mesmo validador puro dos host tests. O novo
  `boot_slot_store_arm` publica exatamente uma tentativa vinculada a lease e à
  geração produzida pelo staging, e um segundo arm não empilha tentativas. O
  planner puro escolhe o slot inativo, exige confirmado ativo e saudável e limita
  o payload à capacidade do alvo. O updater grava os bytes que acabou de
  verificar contra o manifesto assinado, trata slot gravado e não armado como
  recusa, confirma saúde pela geração do token e reporta o rollback aplicado pelo
  loader, desarmando o staged update. `update-stage`/`update-arm on` voltam a ser
  operações de catálogo. A partir desta release `-60` significa apenas transição
  persistente não comprovável. O ciclo completo é provado em host sobre o harness
  provider-backed; a evidência externa com payload assinado + reboot em VMware
  continua aberta, e por isso o critério de update da Etapa 8 permanece aberto.
  Correções de regressão herdadas do trabalho in-tree: `boot_slot.c` acima do
  limite de layout (dividido em `boot_slot_lifecycle.c`) e o fixture de
  `test_audit_events.c` sem o cache de payload. Gates executados:
  `make test`, `make layout-audit` (zero warnings), `make version-audit`,
  `make all64` + `make iso-uefi` (host) e `make release-check` (ELF) — todos com
  exit `0`, ISO final `9ae5f60f...0e47`. Não reexecutados: gates de instalador
  QEMU/VMware e `make modules-index`.

- **`alpha.321` — entrada do instalador robusta em VMs sem COM1:** o VMware
  devolvia `0xFF` ao ler a UART legado ausente; o loader tratava o byte como
  entrada, fixava COM1 como fonte e preenchia o prompt com `ÿ`. A UART agora
  precisa passar por loopback 16550, estados `0x00`/`0xFF`, erros RX e bytes
  nao-tecla sao rejeitados, e a fonte serial so e fixada apos tecla valida.
...
- **Histórico `alpha.319` — gate do ciclo A/B entregue, candidata não
  ...
  update. A interpretação inicial de execução em `.rodata` foi descartada pela
  investigação posterior. A causa real e sua correção estão registradas na
  `alpha.320` abaixo; a Etapa 8 permaneceu aberta.
- **`alpha.320` — regressão de boot/instalação corrigida e comprovada em KVM/TCG:**
  a causa raiz era o uso da red zone SysV pelo loader UEFI. Firmware e
  interrupções podem sobrescrever os 128 bytes abaixo de `RSP`, corrompendo o
  frame antes da entrada do kernel. O loader agora é compilado sem red zone,
  troca para uma pilha real (confirmada no `objdump` do artefato exato), valida o
  ELF, copia e compara os segmentos byte a byte e recarrega integralmente a GDT.
  Dois ciclos KVM focados passaram, com dois boots cada e persistência (quatro
  boots no total). O smoke oficial ISO KVM também passou em 2026-07-30:
  boot #0 instalou pela ISO, boot #1 iniciou o disco instalado e boot #2
  comprovou persistência com `marker:persist-ok`. Em 2026-07-31, o smoke CLI TCG
...
- **Conteúdo do gate entregue no `alpha.319`:** `make
  ...
  conduzem dois ciclos completos em quatro power cycles: manifesto Ed25519
  buscado por HTTP, payload verificado por tamanho + SHA-256, escrita no slot
...
- **Sequencia atual:** Etapa 9 desbloqueada; Etapas 10-16 dependem do fechamento integral da etapa anterior.
...
[`compatibility-audit-2026-07-30-alpha320.md`](../reference/integration/compatibility-audit-2026-07-30-alpha320.md).

| Repo apartado | Versão atual | Owner autoritativo | Gate de integração CapyOS |
|---|---|---|---|
| [`CapyUI`](../../../CapyUI) | `2.24.2` | widget 2.22/schema 7, desktop-session v1, hardening de logout/worker/session scoped e release supply-chain fail-closed | Etapas 4, 6, 7 e gates regressivos da Etapa 8 |
| [`CapyAgent`](../../../CapyAgent) | `0.0.10` | formato `.capypkg`, component-index, resolver, **signer Ed25519 publicado host-side** (`capy-agent-component-index` v1; verifier CapyOS-side registrado + KAT host-validado em `alpha.276`, fail-closed até o trust anchor de produção; KAT externo do signer pendente) | Etapas 8-9 |
...
- Repositório externo **não conta como progresso oficial de etapa** até
  ...
  externo aprovado.
- O adapter in-tree `services/capypkg` é a fronteira de recepção alpha para
  módulos remotos. O signer do CapyAgent foi publicado e o verifier foi
  registrado em `alpha.276`; repos `signed` continuam fail-closed sem chave
  publica de publisher de producao pinada e sem o KAT externo.
...
HTTPS-only no transporte, SHA-256 obrigatório, signature gate fail-closed
(verifier CapyOS-side registrado desde `alpha.276`; trust anchor de produção e
KAT externo do signer ainda pendentes), escopo de filesystem restrito a
`/var/capypkg` ou `/opt/`,
...
- `alpha.261` — **release histórica**: corrige o
...
- `alpha.262` — **release histórica; lote coordenado de 7 repos**:
  ...
  `docs/releases/capyos-0.8.0-alpha.262+20260602.md`. A Fase F passou
  externamente e fechou a Etapa 4.

## Histórico de hardening cross-module (snapshot alpha.262)
...
| 4 | CapyDisplay 2D + scheduler/multithread runtime | Concluída | fechada em alpha.262 |
...
| 9 | Package manager + SDK + ABI estável | **Desbloqueada (próxima)** | dependência da Etapa 8 satisfeita |
...
**Fechamento histórico da Etapa 4:** a **Fase F** passou na VMware oficial
(`make smoke-x64-vmware-etapa-4` + regressões da Etapa 3 + `release-check`) e
fechou a etapa em `alpha.262`.

**Critérios de aceite confirmados; rastreabilidade critério → fase → evidência →
gate em [`active/etapa-4-closure-tracker.md`](active/etapa-4-closure-tracker.md)
§3:**
...
- [x] Widget model desacoplado consegue renderizar display list por
