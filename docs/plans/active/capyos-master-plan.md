...
**Status:** Etapas 1-2 oficialmente fechadas; 2/16 etapas concluídas; Etapa 3 em andamento.
...
| 3 | Driver framework + entrada USB HID + storage estável | Em andamento | Etapa 2 | XHCI/USB HID maduro, AHCI/NVMe estáveis, VirtIO opcional, política fail-safe de driver |
| 4 | CapyDisplay 2D + scheduler/multithread runtime | Bloqueada | Etapa 3 | camada 2D com damage/double buffer, scheduler cooperativo, multithread runtime e contrato widget/display-list |
...
**Status atual:** concluída em `0.8.0-alpha.237+20260514`; validação externa final informada como aprovada pelo operador em 2026-05-18.
**Progresso por escopo fechado:** 100% entregue e aceito operacionalmente.
...
| Dispatcher central, rotas de mouse/teclado e snapshots de prontidão | 14% | 14/14 entregue | regressões cobertas pelos gates finalizados |
| Loginwindow GUI seguro declarativo sem autenticação gráfica | 16% | 16/16 entregue | concluído |
...
| Orquestrador automático da migração legacy -> header-managed | 8% | 8/8 entregue | regressões cobertas pelos gates finalizados |
| Loginwindow GUI real com senha, lockout, recuperação e transição de sessão | 10% | 10/10 entregue | regressões cobertas pelos gates finalizados |
| Smokes reais e evidência de release para `gui-session` e `mouse-events` | 12% | 12/12 entregue por contrato, target, evidência e execução externa | concluído |

**Validação externa concluída para aceite operacional da Etapa 2.**

| Gate externo | Evidência obrigatória | Critério para considerar aceito |
|---|---|---|
| `alpha.237` | Execução fora desta máquina dos gates finais | Operador informou execução bem-sucedida de `make test`, `make layout-audit`, `make all64`, `make release-check`, `make smoke-x64-vmware-mouse-events` e dos gates de readiness/evidência/aceitação/promoção com `RELEASE_TAG=0.8.0-alpha.237+20260514` em 2026-05-18. |

**Fora do escopo de fechamento da Etapa 2.**
CapyDisplay 2D avançado, TLS userland real, apps básicos, package manager,
CapyLX, Wayland, Mesa/Vulkan, JavaScript, CapyLang e hardening 1.0 pertencem
às Etapas 4-16 e continuam bloqueados pela regra sequencial. Framework de
drivers, entrada USB HID e storage estável pertencem à Etapa 3 ativa.
...
**Callers em producao (snapshot historico de alpha.220):**

- `src/installer/installer_main.c:464` — usava
  `crypt_derive_xts_keys` PBKDF2. **Arquivo removido do tree** pela
  higiene total do core (ver
  `docs/reference/integration/core-migration-quarantine.md`); o
  installer modernizado entra como projeto desacoplado.
...
Os callsites remanescentes NAO foram trocados nesta release — a
primitiva esta entregue mas o consumo aguarda design do header de
volume com algorithm marker (sem ele, upgrade do binario do
kernel quebra volumes encrypted com PBKDF2). Slice futuro
definira o header de volume + tools de re-keying.
...
- **Disk salt continua hardcoded** (`g_disk_salt` em
  `installer_main.c:39-41` na época; o arquivo foi removido pela
  higiene total — ver
  `docs/reference/integration/core-migration-quarantine.md` — e
  os callsites remanescentes vivem em `src/core/kernel.c:28` e
  `src/arch/x86_64/kernel_io_helpers.c:53`). Mitigacao: Argon2id
  ainda paga 8 MiB por candidate mesmo com salt fixo, mas
  idealmente seria per-install random. Endereçado no mesmo slice
...
- **3 callsites de `g_disk_salt` continuam existindo como fallback
  legacy** — `src/core/kernel.c` (32-bit legacy aposentado, mantido
  apenas como compat de boot path antigo),
  ...
  volumes legacy em header-managed in-place. O quarto callsite
  histórico (`src/installer/installer_main.c`) foi **removido do tree**
  pela higiene total do core (ver
  `docs/reference/integration/core-migration-quarantine.md`); o
  installer 32-bit não existe mais no repositório e o futuro
  installer modernizado entra como projeto desacoplado.
...
**Status atual:** em andamento após aceite operacional externo da Etapa 2 informado em 2026-05-18. Slices 3A-3D da onda XHCI + USB HID estão entregues por código/testes host-side, incluindo SET_CONFIGURATION, HID boot protocol, Configure Endpoint e polling interrupt no Slice 3D; o próximo gate obrigatório é a validação externa do Slice 3D antes de liberar 3E, conforme o plano subordinado `docs/architecture/etapa-3-driver-foundation-plan.md`.
...
A próxima ação deve continuar somente a **Etapa 3 — Driver framework + entrada USB HID + storage estável**, validando externamente o Slice 3D — SET_CONFIGURATION + HID boot protocol + Configure Endpoint + interrupt transfer + HID polling real. Nenhuma etapa posterior deve começar antes desse gate fechar 100%.
