...
**Fechada em `alpha.312`:** o runtime gráfico estático navega sites reais com
toolbar, endereço, voltar/avançar/reload, redirects, HTML/CSS, imagens, links,
scroll, cache e erros limitados. O smoke QEMU controlado prova redirect + CSS +
imagem + clique + histórico + reload. JavaScript permanece reservado à Etapa
12; formulários avançados e streaming incremental são melhorias posteriores,
não bloqueadores do browser estático solicitado.
...
> **Histórico da abertura em `alpha.288`** (Etapa 6 fechada em alpha.287): esta
> etapa foi concluída em `alpha.312`. O core estático já estava host-testado em
> **CapyBrowser** (`capy-browser-core` v0.6.5: DOM M1, CSS M2,
...
- **7.2 — superfície gráfica ring-3 + raster em pixels (núcleo CONCLUÍDO em `alpha.289`):** ABI de 6 syscalls ring-3 (`SYS_WINDOW_CREATE`/`SURFACE_FILL`/`SURFACE_BLIT`/`WINDOW_POLL_EVENT`/`WINDOW_PRESENT`/`WINDOW_DESTROY`) com **isolamento Opção A** — o app compõe pixels no próprio buffer e faz copy-in validado (bounds/overflow/ownership fail-closed); nunca recebe ponteiro de surface do kernel, referenciando a janela só por um handle opaco rastreado por pid (limpeza automática no exit/reap). Módulo `kernel/syscall_gfx.c` (handlers) com vtable injetável `struct syscall_gfx_ops` + backend de compositor `kernel/syscall_gfx_init.c`; rasterizador CapyOS `userland/bin/capybrowse/browser_render_pixel.{c,h}` que pinta o **mesmo** `capy_dl` em ARGB32 (RECT/TEXT com font 8x8 compartilhada/IMAGE placeholder/LINK underline + parser de cor CSS). Host-testado (handlers via fake backend: ownership, bounds, fail-closed, copy de pixels; rasterizador com assertivas de pixel) + `make all64` linka o backend do compositor. Segundo backend substituível do `capy_dl` (critério 6), agora em pixels.
...
- **7.7 — navegador gráfico como app real numa sessão de desktop viva (alpha.302–309 + trabalho in-tree pós-309):** a Etapa evoluiu do boot-smoke isolado para o `capygfx` coexistindo com o `desktop_runtime_start` real do CapyUI. Entregue e (majoritariamente) provado em QEMU: **alpha.302–303** resolvem a colisão de símbolo `capy_url_parse` (rename cirúrgico) e ligam o fetch de sub-recurso (imagem) por rede através do gate de mixed-content; **alpha.304** cria o spawn não-noreturn `kernel_spawn_capygfx_desktop` (`process_create`+`elf_load_into_process`+arma primeiro dispatch+`scheduler_add`, sem `process_enter_user_mode`), instala o backend de produção do `syscall_gfx` sob demanda e embute o `capygfx` fora do gate de smoke; **alpha.305** entrega a ponte de input (`gfx_owner_pid` na `struct gui_window` + `gfx_owned_redirect` no dispatcher do CapyUI) que re-empurra mouse/teclado/scroll de janelas ring-3 na fila `gui_event`; **alpha.306** torna o trecho crítico do spawn atômico via `cli()/sti()` (mesma garantia dos chamadores de boot, agora fora do boot); **alpha.307** corrige o pager bloqueante do shell gráfico; **alpha.308** promove o navegador gráfico e o loop interativo a padrão no perfil full, faz o `exit` do terminal fechar só a janela e liga o scroll do scrollback; **alpha.309** adiciona o preempt guard do scheduler (`scheduler_preempt_disable/enable`) que torna cada frame do desktop uma unidade atômica de escalonamento (fix da tela azul do VMware).
  - **Trabalho in-tree pós-alpha.309 (rotulado `alpha.310`/`alpha.311` no código; bump de `VERSION.yaml` + gate VMware ainda PENDENTES):** (a) **NXE consistente em todas as CPUs** — BSP (`entry64.S`) e APs (`ap_boot.S`) passam a habilitar `EFER.NXE`; o `capygfx` é o primeiro binário com `.rodata` NX separado e a firmware UEFI da VMware inicia com NXE off → `#PF` de bit reservado sem esse fix (QEMU/OVMF mascarava por iniciar com NXE on); (b) **kernel constrói e possui suas próprias page tables** (identidade 0–16 GiB com páginas huge de 2 MiB, preservando a metade alta do firmware) em vez de herdar as tabelas voláteis do firmware; (c) **usabilidade do navegador**: corrige o poll (`gui_event_poll` retorna `0`/`-1`, nunca `1` — o backend checava `!= 1` e nunca entregava evento nenhum), adiciona **window-gone → CLOSE** (fechamento robusto independente do CLOSE enfileirado), faz `task_sleep` **degradar para yield** quando não há como acordar (host sem tick de APIC — capygfx dormia para sempre e nunca via o CLOSE), e adiciona scroll ao rasterizador (`capyos_browser_render_pixels_scrolled`, `(0,0)` == render original); (d) **engenharia reversa completa da integração browser↔sistema + testes unitários** (esta sessão): novo `tests/kernel/test_task_sleep.c` (predicado `scheduler_can_sleep_current` incl. o caso `total_ticks==0` da VMware + os dois ramos de `task_sleep`), novo `tests/gui/test_syscall_gfx_backend.c` (1ª cobertura host do backend real `syscall_gfx_init.c`: o fix `!= 0`, window-gone→CLOSE repetível, tradução de eventos, filtro de `window_id`), + extensões em `test_syscall_gfx.c` (ownership de poll/blit, overflow de 16 Mpx, sanitização de título) e `test_browser_render_pixel.c` (scroll ↑/←, `(0,0)`==unscrolled, off-screen→bg, fail-closed com offset). `make test` + `make layout-audit` verdes.

### Débitos técnicos e hardening da Etapa 7 (auditoria pós-alpha.309)

Os itens 1, 2, 5 e 7 abaixo foram fechados em `alpha.312` com fila/poll por
janela, guarda de instância/foco, ownership de page tables e teardown/reap de
processos. Os itens restantes são melhorias não bloqueantes do escopo estático.

> Estes itens NÃO bloqueiam o mecanismo já provado (spawn/scheduler/input/close/scroll), mas devem ser fechados antes de declarar o navegador gráfico "usável em campo" e antes do gate `smoke-x64-vmware-browser-https-static`.

1. **[Etapa 7 — antes do gate VMware] Fila de eventos single-consumer (correção nº 1).** `gfx_backend_poll_event` (`src/kernel/syscall_gfx_init.c`) é o único consumidor da fila `gui_event` compartilhada e **retira-e-descarta** qualquer evento de outra janela, além de consumir silenciosamente FOCUS/BLUR/PAINT/TIMER. Latente hoje (uma janela gfx), mas quebra com **2+ janelas ring-3** (uma rouba o input/close da outra) e faz o loop de drain do `capygfx` **parar cedo** (retorno `0` é ambíguo: "vazio" vs. "descartei um alheio"). Fix: fila por janela/handle ou filtrar-no-lugar sem desenfileirar o alheio.
2. **[Etapa 7] Sem guarda de instância única do `capygfx`.** Cada "abrir navegador" cria um processo ring-3 novo (~50 frames de page-table por spawn). Com o teto de 64 tasks, abrir/fechar em sequência pressiona a tabela de tasks e a PMM. Apps in-kernel já fazem "focar se já aberto"; o navegador ring-3 precisa do mesmo guard.
3. **[Etapa 7] `task_sleep` degradado vira busy-spin ~100% CPU** em host sem tick, e há risco de **trava** se o APIC ticar e depois parar (o guard só checa `total_ticks==0`; um `wake_tick` já agendado nunca seria alcançado). Endurecer o guard (ex.: detectar "tick parou há N") e considerar reduzir o spin.
4. **[Etapa 7] Fail-safe morto.** `CAPYGFX_DESKTOP_POLL_MAX_ITERS` está definido em `userland/bin/capygfx/main.c` mas **nunca é referenciado** no loop interativo — o loop não tem teto de iterações (mitigado pelo window-gone→CLOSE, mas o fail-safe documentado não existe). Reativar ou remover o define.
5. **[Etapa 7/8] Frames de page-table do firmware não são reservados explicitamente na PMM.** Após o kernel assumir as próprias tabelas, os frames de tabela do firmware ficam protegidos só incidentalmente (não-tipo-7 conventional + scan de read-only). Adicionar reserva explícita (ou um assert `pmm_alloc_page() != CR3-do-firmware`) em `vmm_build_kernel_tables` — é a fronteira delicada por trás de toda a saga de travas VMware.
6. **[Etapa 7] Ponte de input só existe no dispatcher do CapyUI** (`gfx_owned_redirect`), não no fallback in-tree `src/gui/window/window_dispatcher.c` — um build sem o sibling perde silenciosamente o input do navegador. Documentar como limitação conhecida ou portar o choke point.
7. **[Etapa 7] Reaping depende do `capygfx` ser órfão** (desktop não roda numa thread de processo). Um `capygfx` parented viraria um zumbi que vaza slot de processo + address space (sem init/PID 1, sem `wait()`). Garantir/documentar a invariante do órfão ou dar um dono que colha.
8. **[transversal] Sem cobertura host do caminho de spawn/clone-de-AS/build-de-tabelas/`elf_load`** — é smoke-only por natureza (precisa de MMU real). Manter o smoke de respawn (`CAPYOS_ELFLOAD_RESPAWN_SMOKE`, contagem de `free_pages` monotônica) como detector de vazamento e observar o `gfxspawn_diag` no COM1 no protocolo de abrir/fechar N vezes.

> **Sinal de campo restante:** o "freeze cumulativo do host VMware após muitos ciclos abrir/fechar" (sem panic do guest) é consistente com a combinação dos débitos 2 + 3 (+ possivelmente 5) — não é pego por teste unitário. Protocolo de reteste: abrir/fechar o navegador algumas vezes observando o `gfxspawn_diag` no COM1; `free_pages` não-monotônico = vazamento (então priorizar o débito nº 2).
...
- [x] Páginas alvo (wikipedia, blog estático, docs, search engine simples, news estático) carregam e renderizam sem travar a UI.
- [x] HTTPS válido carrega; HTTPS inválido falha fechado com mensagem clara.
- [x] Imagens JPEG/PNG aparecem inline.
- [x] Cache acelera segunda visita observavelmente.
- [x] Modo texto continua disponível quando render gráfico, imagem ou CSS falha.
- [x] Parser/layout/display-list permanecem substituíveis sem acoplar core puro ao compositor.
...
- Fingerprint real da chave offline que assina checksums versionado em
  `.github/release-policy/release-checksum-ed25519.sha256`; a chave distinta do
  update-agent continua pinada no runtime.
- CI executa smoke VMware+E1000 com DHCP/DNS/HTTP/HTTPS.
- `update-fetch` e payload HTTPS passam em ambiente controlado.
- Release gate valida o draft autenticado antes da promoção e produz a evidência
  pública pelas URLs imutáveis somente depois da mutação atômica.
...
um alvo elegível. Nesta fatia, o critério do wizard permaneceu aberto até o gate
VMware multi-disco registrado em `alpha.316` provar fresh install, reboot, login,
persistência e disco guard intacto.
...
**Entregue em `alpha.317` — fundação A/B fail-closed e handoff v9:** o
formato interno v0 mantém duas cópias de 512 bytes com CRC32, geração monotônica,
campos reservados zerados e commit alternado com `write` + `flush` + readback
integral. O planner divide o BOOT oficial de 524.288 setores em duas regiões
fixas, preserva manifest/payload A em LBA relativo 0/1 e reserva os dois setores
finais para controle. Cada slot vincula header/payload LBA, capacidade, tamanho
exato, versão ASCII e SHA-256; o reopen exige a geometria fornecida pelo provider,
não confia no próprio registro. Estados `confirmed`, `pending` e
`tries_remaining` são explícitos: a seleção consome a única tentativa antes do
boot, confirmação exige slot + geração exatos e o próximo select sem confirmação
faz rollback no metadata antes de devolver o confirmed. Split-brain, mirror
pendente degradado, store vazio versus I/O, SHA ausente, versão não imprimível e
resultado pós-write indeterminado falham fechados. Nesta fatia
`update-rollback-check` permanecia incondicionalmente em `-60`, inclusive diante
do provider v0, para não promover simulação de metadata a rollback real; o
`alpha.318` o liga ao resultado durável.

O `boot_slot_store` valida snapshot estável + geometria e só aceita sobrescrever
um slot inativo em estado `EMPTY` ou `FAILED`; nesta fatia um slot `VALID` era
recusado antes de qualquer I/O por não existir ainda uma transição de invalidação
durável (o `alpha.318` fechou essa lacuna: a autorização de staging grava
`FAILED` nos dois espelhos de controle antes de tocar um byte de payload, então
restage de slot `VALID` passou a ser legal). Em seguida escreve
o payload, exige flush, relê todos os setores, recalcula SHA-256 e valida todo o
padding final antes de gravar o header v0 por último; o segundo flush/readback
valida magic, versão, LBA, capacidade, tamanho, SHA, CRC e reserved-zero. Falhas
após write viram resultado indeterminado e nunca alteram o manager confirmed.

O `boot_slot_block_provider` valida headers e arrays primary+backup, CRCs e
igualdade byte a byte entre os dois arrays, aceita somente o layout canônico
ESP/BOOT/DATA sem partições extras, exige GUIDs únicos, atributos zero e binding
exato das faixas ESP/BOOT/DATA do disco montado; então mapeia LBAs relativos do
store para o BOOT raw com bounds e flush obrigatório. O harness sparse inclui um
vetor de arrays divergentes com CRC32 colidente, prova que a divergência é
recusada e que payload/header atingem apenas o slot B absoluto enquanto A fica
intacto. O handoff v9 append-only preserva o prefixo v8 de 440 bytes e acrescenta
ranges+GUIDs de ESP/BOOT/DATA. O loader zera os 560 bytes, aceita exatamente uma
flag de identidade strict ou legacy, vincula raw/ESP/DATA ao mesmo prefixo físico
de Device Path, centraliza I/O destrutivo com `Media->IoAlign`, propaga toda falha
de `FlushBlocks`, valida o manifest BOOT sem type-punning e seleciona exatamente
um raw/handle por GUID; discos alpha.316 e anteriores com
GUIDs duplicados entram por parser legado validado, sem capability de update. O
runtime nativo enumera todos os backends, exige exatamente um fingerprint
correspondente, retém raw+DATA e revalida a mesma identidade antes do mount;
ranges 64-bit, clipping, troca de mídia e ambiguidade falham fechados.

Após mount, a capability exige raw 512/read/write, DATA+ESP estritos e flush.
O layout GPT permanece inalterado.

**Entregue em `alpha.318` — apply A/B persistente habilitado:** o flush durável
já existe nos três backends nativos (NVMe `FLUSH`, AHCI/ATA `FLUSH CACHE`/`EXT`
descoberto por IDENTIFY word 83) e é exigido pelo provider antes de qualquer
registro, então a capability deixa de ser zerada e passa a ser negada apenas por
motivo declarado (`NO_FLUSH`, `NO_CONTROL`, `CONTROL_UNKNOWN`,
`TOKEN_MISMATCH`). O handoff avança para v10: além da identidade GPT do v9, o
prefixo de 560 bytes é congelado e são acrescentados 16 bytes de token de
tentativa (`flags`, `slot`, `generation`) com combinações canônicas fechadas.
O loader é autoritativamente A/B — seleciona o slot, consome a única tentativa
com commit durável, valida header/payload/SHA-256/padding antes de carregar,
tenta o rollback quando o slot pendente não carrega e publica o token no
handoff. `kernel_main` valida esse token pelo mesmo validador puro dos host
tests e falha fechado antes de qualquer consumo.

`boot_slot_store_arm` completa o lifecycle: exige binding provider-backed, lease
exata, geração exata do staging e ausência de staging em curso, e então promove
o slot inativo `VALID` a pendente com `tries_remaining = 1`, marcando o
confirmado como `ROLLBACK`. Um segundo arm não empilha tentativas. O planner puro
`x64_storage_boot_provider_plan_stage` escolhe o slot inativo a partir do
snapshot, exige que o confirmado seja o ativo e saudável, recusa attempt pendente
ou rollback armado e limita o payload à capacidade do slot alvo. As pontes
`x64_storage_runtime_stage_boot_payload_sha256`, `_arm_boot_slot` e
`_boot_rollback_check` ligam o updater a esse store usando o mesmo snapshot para
planejar e autorizar.

O updater grava exatamente os bytes que verificou contra o manifesto assinado,
em vez de reabrir o cache num segundo passe. `update-apply` faz stage e arm como
passos explícitos: um slot gravado e não armado é recusa (`-60`), nunca sucesso.
`update-confirm-health` confirma a geração do token e só então limpa o staged
catalog; `update-rollback-check` reporta o resultado durável e desarma o staged
update quando o boot atual já é o rollback aplicado pelo loader. `update-stage` e
`update-arm on` voltam a ser operações de catálogo (`-49`/`-5`/`-10`) e nunca
tocam boot slot. A partir desta fatia `-60` significa apenas transição
persistente não comprovável: sem provider durável, geração/lease obsoleta,
payload não verificado ou commit indeterminado.

O ciclo completo — stage, arm de tentativa única, select que consome a
tentativa, confirmação durável e rollback de tentativa não confirmada — é
...
release stable `0.9.0`, o índice imutável e seus nove módulos HTTPS foram
publicados e validados remotamente por URL, tamanho e SHA-256. A release ainda
não contém `.sig`, manifestos públicos ou `latest.ini`, portanto integridade
remota não equivale à autenticação Ed25519 exigida para o update de produção. O
conjunto coordenado usa CapyUI `2.24.2`, patch de supply chain sobre `2.24.1`,
sem alterar `capy-ui-widget` v2.22, desktop-session v1 ou display-list schema 7.
...
Ed25519 buscado por HTTP, payload verificado por tamanho + SHA-256, escrita no
slot inativo com autorização geracional e flush durável, consumo da única
...
`.github/release-policy/release-checksum-ed25519.sha256`, manifestos,
`latest.ini`, tag no commit exato de `main` e releases imutáveis habilitadas,
comprovadas por `CAPYOS_RELEASE_POLICY_AUDIT_TOKEN`: PAT/App fine-grained de
curta duração, limitado ao repositório e com `Administration: write`. Esse
privilégio elevado é necessário para a API expor `bypass_actors`, embora a
workflow use somente `GET`; resposta parcial falha fechado e a credencial deve
...
exige pull request. O perfil permanente solo exige exatamente zero aprovação,
squash-only, threads resolvidas e seis checks strict autenticados (`Lint`,
`Release gates`, `QEMU ISO smoke`, dois `Analyze` e `CodeQL`). Pin ausente,
placeholder, formato
...
download público sem token. Builder e promoter compartilham um lock global
entre todas as tags; o builder recusa substituir payload depois que material
assinado foi anexado, e o promoter pode retomar apenas a revalidação da mesma
release imutável já
Latest. O ruleset de `main` prova a proteção vigente, não que o commit da tag
tenha passado historicamente por PR/review. Como o lock não cobre ações humanas,
...
publicada como predecessor, recusa chave privada/flags lab, valida assinatura,
...
Ed25519 descartável específico para esse manifesto e compila o kernel com
`CAPYOS_UPDATE_LAB_TRUST_KEY_HEX`; ele é distinto da chave que assina os
checksums da release. O mesmo flag pina o catálogo em
`CAPYOS_UPDATE_LAB_MANIFEST_URL` e libera `payload_url` `http://`, porque a pilha
TLS do kernel sempre verifica o peer e um servidor hermético não apresenta
certificado publicamente confiável. Um único switch controla as três relaxações.
O kernel de laboratório é fail-closed por construção: hex malformado não cai de
volta na chave de produção, `iso-uefi` recusa o ELF fora do próprio target de
smoke, `release-check` reprova qualquer kernel de release que carregue o banner
`[lab] update trust anchor overridden` e o fingerprint de variante invalida
kernel+userland ao voltar para a build oficial. O que permanece como passo de
operador é a custódia das duas chaves oficiais: anexar os cinco materiais
offline e repetir o roteiro numa instalação oficial
([`../../operations/etapa-8-signed-update-playbook.md`](../../operations/etapa-8-signed-update-playbook.md), §4).

Observabilidade que faltava para diagnosticar o `-60`:
`x64_storage_boot_provider_reason_label` nomeia as nove recusas do provider,
`print-boot-slot` imprime `Boot provider: ready=/reason=` e o boot log ganha
`[boot] A/B attempt slot=/state=/generation=` derivado do token validado — a
decisão A/B do loader sai pela console de firmware, que o contrato VMware mantém
fora da COM1, então o kernel reafirma o token no log que ele mesmo controla.
Correções pareadas: `update-rollback-check` decidia a linha `[ok]` por `rc > 0`,
que o agente nunca retorna, e por isso anunciava "nenhum rollback pendente"
logo após um rollback real (novo campo `rollback_applied`); `http_get` mandava
todo host para o resolvedor e falhava com `HTTP_ERR_DNS` em URL com IP literal
(novo `http_parse_ipv4_literal`, estrito e host-testado); e os textos de ajuda de
`update-prepare`/`update-stage`/`update-apply` ainda diziam "indisponível",
desatualizados desde o `alpha.318`.
...
- `make smoke-x64-vmware-installer-wizard`.
- `make smoke-x64-vmware-update-ab` — gate VMware do mecanismo A/B com chave
  descartável e endpoint hermético de laboratório; `make
  smoke-x64-qemu-update-ab` como preflight e `make update-ab-selftest` (dentro
  de `release-check`) como contrato host. Esse target não usa as chaves nem as
  URLs públicas de produção.
- `make smoke-x64-vmware-update-ab-production-existing-iso` — aceite oficial
  pos-promocao com a ISO predecessora publicada, hash pinado e assets publicos;
  recusa qualquer chave privada ou flag de laboratorio.
- `make release-check` como baseline de build/checksums; este target não prova
  payload assinado nem apply A/B, mas reprova um kernel com âncora de
  laboratório.
- Passo de operador com as duas chaves offline: anexar ao draft `.sig`, chave
  pública, os dois manifestos públicos e `latest.ini` (o `capyos64.bin` já é um
  dos sete assets-base), executar a promoção dos 12 assets e, numa instalação
  oficial pós-promoção, aplicar sem confirmar e observar o rollback; depois,
  ja no predecessor restaurado, reaplicar o mesmo release e executar
...
- O índice default do first-boot continua sendo um **pin de release explícito em
  ...
  `src/config/first_boot/modules.c`), agora alinhado ao candidato `0.9.2`; o
...
Estes gates valem para **CapyUI, CapyBrowser, CapyCodecs, CapyAgent, CapyAI,
CapyLang e CapyBenchmark** e são pré-condição para qualquer promoção de módulo a integração
oficial de Etapa:
...
### 20.1 CapyOS (core) — `0.9.2+20260826`
...
- Estado compartilhado sem lock (fila `gui_event`, tabela de janelas do
  compositor) só é seguro sob a disciplina do preempt guard do frame do desktop
  (`alpha.309`); qualquer novo produtor/consumidor fora dessa disciplina precisa
  de fila própria ou lock — ver os débitos técnicos da Etapa 7 (§10) registrados
  pela RE pós-`alpha.309` (fila single-consumer, instância única do `capygfx`,
  guard do `task_sleep` em host sem tick, reserva dos frames de page-table do
  firmware), que devem ser fechados antes do gate VMware do navegador gráfico.
...
- **Entregue (2.22.7–2.23.1, Etapa 7):** ponte de eventos de input para janelas
  ring-3 (`gfx_owned_redirect`, `gfx_owner_pid`), launcher com registro de apps +
  entrada "Navegador" (hook `kernel_desktop_open_browser_graphical`), `exit` do
  terminal fecha só a janela + scroll de scrollback, e o **preempt guard no frame
  do desktop** (`scheduler_preempt_disable/enable`, exige CapyOS ≥ alpha.309) que
  torna o frame uma unidade atômica de escalonamento — fix da tela azul do VMware
  ao coexistir com o `capygfx` ring-3.
...
- **Entregue:** URL + HTML-to-text + links + modelo de erro/warning
  ...
  inalterado).
- **Etapa 7 entregue:** core estático textual/gráfico integrado por adapters
  versionados; streaming/incremental parse e nós adicionais são evolução futura.
...
- **Entregue na Etapa 7:** TaskPlan, grants/audit, adapters tipados de arquivo,
