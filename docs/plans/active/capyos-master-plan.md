...
**Versão atual:** `0.8.0-alpha.255+20260521`
...
**Status:** **Em andamento** desde 2026-05-21 (alpha.253). Etapa 3 fechou externamente nessa mesma data. **Fase A revertida em alpha.255** (alpha.254 foi rolled back): o scaffolding entregue tinha um erro de design (criou ABI paralela em vez de consumir a ABI real do sister `CapyUI`, que já está em v2.7 com display-list schema v7). Próximo passo bloqueador: Fase A correta — implementar adapter CapyOS-side que CONSUMA `CapyUI/src/widget/capy_display_list.h` (struct nativo `capy_dl_cmd`, schema v7), combinado com sincronização cross-repo completa via workflow `cross-repo-contract-sync`.
...
A Etapa 3 fechou formalmente em 2026-05-21 (alpha.253) após validação externa do gate `make smoke-x64-vmware-storage-resilience` em VMware oficial. A Etapa 4 abriu em sequência mas o scaffolding entregue em alpha.254 foi rolled back em **alpha.255** após descoberta de que a ABI `capy-ui-widget` já está publicada pelo sister `CapyUI` em v2.7 (display-list schema v7) — invocar workflow `cross-repo-contract-sync` ANTES de qualquer scaffolding novo de Fase A é obrigatório para sincronizar a matriz cross-repo (que continua stale: pina CapyUI 0.7.3 enquanto o sister real está em 2.7.0). Próxima ação: invocar `cross-repo-contract-sync` para sincronizar matriz + audit + bump dos pins; em seguida, Fase A correta = adapter CapyOS-side que **consume** `CapyUI/src/widget/capy_display_list.h` (não um schema paralelo). Slices 3F-3J e sub-slices 3E.4.C/3E.5.B continuam como follow-ups não-bloqueantes da Etapa 3. Runbook completo da Etapa 4: `docs/operations/etapa-4-external-validation-playbook.md`.
