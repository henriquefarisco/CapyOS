...
|---|---|---|---|---|
| `CapyOS` | `0.8.0-alpha.307+20260701` | `capyos-base` v3 + `capyos-package-apply` v1 | — (autoritativo) | — |
...
| `CapyBrowser` | `0.6.6` | `capy-browser-core` v1 text subset publicado (`org.capyos.browser.text`; URL + HTML-to-text + links + erro/warning deterministico; refs numericas WHATWG -> U+FFFD; `<pre>` preserva whitespace; marcadores de lista); core grafico (display-list) consumido pelo backend de render do CapyOS na Etapa 7 -- desde `0.6.6` o no IMAGE carrega o `src` resolvido (em `url_off`/`url_len`; aditivo, `CAPY_DL_VERSION` inalterado) | `0.6.6` (Etapa 7 / Slice 7.4; rasterizador CapyOS-side consome o display-list + src e decoda via `capy-codec-image`) | `0.6.6` |
...
| `CapyUI` | `2.23.1` | `capy-ui-widget` v2.22 (display-list schema v7) + `capy-ui-desktop-session` v1 (+ apps-basic-roundtrip smoke surface, Slice 6.6; 2.22.7: input bridge `gfx_owned_redirect` para janelas ring-3 gfx-owned, Slice 7.5 — regressão travada por `test_gui_window_dispatcher`; 2.23.0: launcher com registro único de apps + entrada "Navegador" (hook `kernel_desktop_open_browser_graphical`) + toggle "Lista completa"; `exit` no terminal fecha só a janela; scroll de scrollback no terminal; **2.23.1: frame do desktop vira unidade atômica de escalonamento via preempt guard do scheduler (`scheduler_preempt_disable/enable`, CapyOS ≥ alpha.309) — fix da tela azul do VMware ao coexistir com o capygfx ring-3**) | `2.23.1` (cross-repo build / capypkg modulo / display-list adapter Etapa 4; a suíte `gfx-owned redirect` do CapyOS exige ≥2.22.7; o hook do launcher exige CapyOS ≥ alpha.308; o preempt guard exige CapyOS ≥ alpha.309) | `2.23.1` (CI release gates com sibling remoto em main) |
...
|---|---|---|---|
| `CapyOS` | `0.8.0-alpha.307+20260701` (alpha extended) | `v<x>.<y>.<z>+<build>` | autoritativo; assina release quando o gate `release-check` aceitar |
