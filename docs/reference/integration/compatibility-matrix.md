...
| `CapyUI` | `2.23.0` | `capy-ui-widget` v2.22 (display-list schema v7) + `capy-ui-desktop-session` v1 (+ apps-basic-roundtrip smoke surface, Slice 6.6; 2.22.7: input bridge `gfx_owned_redirect` para janelas ring-3 gfx-owned, Slice 7.5 — regressão travada por `test_gui_window_dispatcher`; **2.23.0: launcher com registro único de apps + entrada "Navegador" (hook `kernel_desktop_open_browser_graphical`) + toggle "Lista completa"; `exit` no terminal fecha só a janela; scroll de scrollback no terminal (roda + PgUp/PgDn) — tudo aditivo**) | `2.23.0` (cross-repo build / capypkg modulo / display-list adapter Etapa 4; a suíte `gfx-owned redirect` do CapyOS exige ≥2.22.7; o hook do launcher exige CapyOS ≥ alpha.308) | `2.23.0` (CI release gates com sibling remoto em main) |
