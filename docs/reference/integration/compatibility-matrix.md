...
| `CapyUI` | `2.22.7` | `capy-ui-widget` v2.22 (display-list schema v7) + `capy-ui-desktop-session` v1 (+ apps-basic-roundtrip smoke surface, Slice 6.6, alpha.278-281: 5 apps basicos; task_manager smoke corrigido p/ run queue vazia em alpha.281; **2.22.7: input bridge `gfx_owned_redirect` para janelas ring-3 gfx-owned, Slice 7.5 — aditivo, regressão travada por `test_gui_window_dispatcher`**) | `2.22.7` (cross-repo build / capypkg modulo / display-list adapter Etapa 4; a suíte `gfx-owned redirect` do CapyOS exige ≥2.22.7) | `2.22.7` (CI release gates com sibling remoto em main) |
