...
void terminal_init(struct terminal *term, struct gui_window *win) {
  uint32_t glyph_width = FONT_GLYPH_WIDTH;
  uint32_t glyph_height = FONT_GLYPH_HEIGHT;
  if (!term) return;
  term_memset(term, 0, sizeof(*term));
  term->window = win;
  term->font = font_default();
  if (term->font) {
    if (term->font->glyph_width != 0) glyph_width = term->font->glyph_width;
  ...
  if (win && term->font) {
    term->cols = glyph_width ? (win->frame.width / glyph_width) : 80;
    term->rows = glyph_height ? (win->frame.height / glyph_height) : 25;
  ...
  if (term->cols == 0) term->cols = 80;
  if (term->rows == 0) term->rows = 25;
...
static const uint32_t ansi_colors[8] = {
  0x00000000, 0x00CC3333, 0x0033CC33, 0x00CCCC33,
...
static void ansi_process(struct terminal *term) {
  int val = 0;
  for (int i = 0; i < term->ansi_len; i++) {
    char ch = term->ansi_buf[i];
    if (ch >= '0' && ch <= '9') {
      val = val * 10 + (ch - '0');
    } else if (ch == ';' || ch == 'm') {
      ansi_apply_sgr(term, val);
      val = 0;
    }
  }
}
...
void terminal_write_char(struct terminal *term, char c) {
  ...
  if (term->ansi_state == 1) {
    if (c == '[') { term->ansi_state = 2; term->ansi_len = 0; return; }
    term->ansi_state = 0;
  } else if (term->ansi_state == 2) {
    if ((c >= '0' && c <= '9') || c == ';') {
      if (term->ansi_len < 15) term->ansi_buf[term->ansi_len++] = c;
      return;
    }
    if (c == 'm') {
      term->ansi_buf[term->ansi_len++] = 'm';
      ansi_process(term);
      term->ansi_state = 0;
      return;
    }
  ...
  if (c == '\x1B') { term->ansi_state = 1; return; }
