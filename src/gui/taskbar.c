...
static void menu_popup_paint(struct gui_window *win) {
  ...
  for (uint32_t i = 0; i < tb->menu_entry_count; i++) {
    int32_t ey = (int32_t)(i * TASKBAR_MENU_ENTRY_HEIGHT) + 2;
    font_draw_string(s, f, 12, ey + 6, tb->menu_entries[i].label, theme->text);
    /* Separator line below each entry */
    if (i + 1 < tb->menu_entry_count)
      tb_fill_rect(s, 4, ey + (int32_t)TASKBAR_MENU_ENTRY_HEIGHT - 1,
                   s->width - 8, 1, theme->window_border);
  }
}
...
void taskbar_toggle_menu(struct taskbar *tb) {
  ...
  if (tb->menu_open) {
    /* Create the popup window if it doesn't exist yet */
    if (!tb->menu_popup && tb->menu_entry_count > 0) {
      uint32_t popup_h = tb->menu_entry_count * TASKBAR_MENU_ENTRY_HEIGHT + 4;
      int32_t popup_y = 0;
      if (tb->window)
...
int taskbar_handle_menu_click(struct taskbar *tb, int32_t screen_x,
  ...
  uint32_t idx = (uint32_t)(local_y / (int32_t)TASKBAR_MENU_ENTRY_HEIGHT);
  if (idx < tb->menu_entry_count) {
    struct taskbar_menu_entry *e = &tb->menu_entries[idx];
    taskbar_toggle_menu(tb);  /* Close menu first */
    if (e->action) e->action(e->user_data);
    return 1;
  }
