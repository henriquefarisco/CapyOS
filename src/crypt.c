...
static void sha256_transform(struct sha256_ctx *ctx, const uint8_t data[SHA256_BLOCK_SIZE]) {
    uint32_t m[64];
...
static void sha256_final(struct sha256_ctx *ctx, uint8_t hash[SHA256_DIGEST_SIZE]) {
...
void crypt_derive_xts_keys(const char *password,
    ...
    pbkdf2_hmac_sha256((const uint8_t *)password, pass_len,
                       salt, salt_len,
                       iterations,
...
static void inv_mix_columns(uint8_t *state) {
    for (uint8_t i = 0; i < 4; ++i) {
        ...
        column[0] = multiply(a0, 0x0e) ^ multiply(a1, 0x0b) ^ multiply(a2, 0x0d) ^ multiply(a3, 0x09);
        column[1] = multiply(a0, 0x09) ^ multiply(a1, 0x0e) ^ multiply(a2, 0x0b) ^ multiply(a3, 0x0d);
        column[2] = multiply(a0, 0x0d) ^ multiply(a1, 0x09) ^ multiply(a2, 0x0e) ^ multiply(a3, 0x0b);
        column[3] = multiply(a0, 0x0b) ^ multiply(a1, 0x0d) ^ multiply(a2, 0x09) ^ multiply(a3, 0x0e);
    }
}

static void aes_encrypt_block(const struct aes_ctx *ctx, const uint8_t input[16], uint8_t output[16]) {
    uint8_t state[16];
    for (uint8_t i = 0; i < 16; ++i) {
        state[i] = input[i];
    }
...
static void aes_decrypt_block(const struct aes_ctx *ctx, const uint8_t input[16], uint8_t output[16]) {
    uint8_t state[16];
    for (uint8_t i = 0; i < 16; ++i) {
        state[i] = input[i];
    }
...
struct block_device *crypt_init(struct block_device *lower,
    ...
    struct crypt_device *crypt = (struct crypt_device *)kalloc(sizeof(struct crypt_device));
    if (!crypt) {
        return NULL;
    }
