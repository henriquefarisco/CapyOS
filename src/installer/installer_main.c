...
void kernel_main(uint32_t mb_magic, uint32_t mb_info_ptr) {
  ...
  // Cria estrutura minima de diretorios no volume recem-formatado.
  // O assistente de configuracao completo (usuario admin, tema, hostname)
  // sera executado automaticamente no primeiro boot a partir do disco.
  {
    const char *base_dirs[] = {"/bin", "/docs", "/etc", "/home",
                               "/system", "/tmp", "/var", "/var/log"};
    for (size_t i = 0; i < sizeof(base_dirs) / sizeof(base_dirs[0]); ++i) {
      vfs_create(base_dirs[i], VFS_MODE_DIR, NULL);
    }
    vga_write("Estrutura de diretorios base criada.\n");
  }
