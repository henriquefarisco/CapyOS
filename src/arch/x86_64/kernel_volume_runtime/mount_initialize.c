...
int mount_root_capyfs(struct x64_kernel_volume_runtime_state *state,
  ...
  int mount_rc = 0;
  int root_rc = -1;
  if (!state || !state->root_sb) return -1;
  ...
  *state->active_volume_key_ready = 1;
  ...
  if (fmt_rc != 0) {
    ...
    if (buffer_cache_last_error_block(&sync_block) == 0) {
      ...
      io_print_dec_u32(io, (uint32_t)(buffer_cache_last_error_code() & 0xFFFFFFFF));
    }
  ...
  if (state->data_io_probe && state->data_io_probe_size >= data_dev->block_size &&
    ...
    for (uint32_t i = 0; i < data_dev->block_size; ++i) {
      if (state->data_io_probe[i] != 0) ++nonzero;
    }
