...
static int config_line_equals(const char *line, size_t len, const char *key, const char *value) {
    if (!line || !key || !value) {
        return 0;
    }
...
static void bytes_to_hex_str(const uint8_t *src, size_t len, char *dst, size_t dst_size) {
...
static void log_emit_segments(const char *a, const char *b, const char *c, const char *d, const char *e) {
...
static int verify_config_file(const char *hostname, const char *theme, int splash_enabled) {
    ...
    for (size_t i = 0; i <= (size_t)read; ++i) {
        if (i == (size_t)read || buffer[i] == '\n') {
            size_t len = i - start;
            if (len > 0) {
                if (!hostname_ok && config_line_equals(&buffer[start], len, "hostname", hostname)) {
                    hostname_ok = 1;
                } else if (!theme_ok && config_line_equals(&buffer[start], len, "theme", theme)) {
                    theme_ok = 1;
                } else if (!splash_ok && config_line_equals(&buffer[start], len, "splash", splash_value)) {
                    splash_ok = 1;
                }
            }
            start = i + 1;
    ...
    if (!hostname_ok || !theme_ok || !splash_ok) {
...
static size_t wizard_prompt(const char *prompt, char *buffer, size_t buffer_len, int secret) {
...
static int prompt_password_pair(const char *label, char *password, size_t password_len) {
    for (int tries = 0; tries < 3; ++tries) {
        ...
        size_t clen = wizard_prompt("Confirme a senha: ", confirm, sizeof(confirm), 1);
        if (plen == 0 || clen == 0) {
...
static const char *g_cli_reference_text =
    ...
    "  find ""texto"" [onde]    - procura conteudo\n"
    "\n"
    "Processos & Sistema\n"
...
static const char *validate_theme(const char *input) {
    ...
    if (strings_equal(input, "noir") || strings_equal(input, "ocean") || strings_equal(input, "forest")) {
        return input;
    }
    return "noir";
}
...
int system_run_first_boot_setup(void) {
    ...
    if (write_text_file("/docs/noiros-cli-reference.txt", g_cli_reference_text) != 0) {
        print_line("   Aviso: nao foi possivel gravar referencia do NoirCLI.");
    } else {
        print_line("   Referencia NoirCLI pronta em /docs/noiros-cli-reference.txt.");
    }
    sync_root_device();
    ...
    size_t hlen = wizard_prompt("Hostname [noiros-node]: ", hostname, sizeof(hostname), 0);
    if (hlen == 0) {
    ...
    size_t tlen = wizard_prompt("Tema [noir]: ", theme_input, sizeof(theme_input), 0);
    if (tlen == 0) {
    ...
    size_t slen = wizard_prompt("Ativar splash animado? [S/n]: ", splash_answer, sizeof(splash_answer), 0);
    int splash_enabled = 1;
    if (slen > 0 && (splash_answer[0] == 'n' || splash_answer[0] == 'N')) {
        splash_enabled = 0;
    }
    print_line(splash_enabled ? "   Splash animado: habilitado" : "   Splash animado: desabilitado");
    ...
    if (vfs_set_metadata(admin_home, &home_meta) != 0) {
        print_line("Aviso: nao foi possivel ajustar permissoes do diretorio pessoal.");
    }
    sync_root_device();
    ...
    if (userdb_find(admin_username, &existing) == 0) {
        ...
        if (verify_directory_exists(existing.home) != 0) {
            ...
            rebuild_msg[0] = '\0';
            buffer_append(rebuild_msg, sizeof(rebuild_msg), "   Diretorio pessoal ausente. Recriando em ");
            ...
            if (ensure_directory(existing.home) != 0 || verify_directory_exists(existing.home) != 0) {
    ...
    while (!admin_ready) {
        if (prompt_password_pair("Defina a senha para o usuario admin: ", admin_password, sizeof(admin_password)) != 0) {
        ...
        if (user_record_init(admin_username, admin_password, "admin", admin_uid, admin_gid, admin_home, &admin) != 0) {
        ...
        if (userdb_authenticate(admin_username, admin_password, &verify_rec) != 0) {
            print_line("Falha no teste de autenticacao do administrador. Recriando base de usuarios.");
    ...
    sync_root_device();
    if (verify_config_file(hostname, theme, splash_enabled) != 0) {
        return -1;
    }
    ...
    admin_summary[0] = '\0';
    buffer_append(admin_summary, sizeof(admin_summary), "Administrador configurado: ");
    ...
    if (userdb_find(admin_username, &final_rec) == 0) {
        ...
        print_line("   Aviso: nao foi possivel reler registro admin apos configuracao.");
    }
...
static void apply_config_line(struct system_settings *settings, const char *line, size_t len) {
    size_t eq = 0;
...
int system_load_settings(struct system_settings *out) {
    ...
    cstring_copy(out->hostname, sizeof(out->hostname), "noiros-node");
    cstring_copy(out->theme, sizeof(out->theme), "noir");
    out->splash_enabled = 1;
...
int system_login(struct session_context *session, const struct system_settings *settings) {
    ...
    while (1) {
        ...
        attempt_msg[0] = '\0';
        buffer_append(attempt_msg, sizeof(attempt_msg), "Login tentativa para usuario=");
        ...
        if (userdb_authenticate(username, password, &record) == 0) {
            ...
            success_msg[0] = '\0';
            buffer_append(success_msg, sizeof(success_msg), "Login bem-sucedido: usuario=");
