...
size_t hv_skip_block(const char *html, size_t len, size_t pos,
                            const char *tag) {
  int depth = 1;
  while (pos < len) {
    char inner[32];
...
size_t hv_collect_text_until_tag(const char *html, size_t len,
...
static int hv_flush_inline_text(struct html_document *doc,
  ...
  *node = *tmpl;                /* copy CSS, indent, color, etc. from parent */
  node->type = preserve_block_type ? tmpl->type : HTML_NODE_TEXT;
...
size_t hv_parse_inline_content(const char *html, size_t len, size_t pos,
  ...
  size_t start_node_count = doc ? doc->node_count : 0;
  size_t buf_pos = 0;
  int split_inline = 0;
  int last_space = 1;
  buf[0] = '\0';
  ...
  while (pos < len) {
    ...
    /* Tag */
    {
      ...
      if (hv_streq_ci(inner, "br") || hv_streq_ci(inner, "wbr")) {
        split_inline = 1;
        hv_flush_inline_text(doc, tmpl, buf, &buf_pos, 0);
  ...
  hv_flush_inline_text(doc, tmpl, buf, &buf_pos,
                       !split_inline && doc &&
                           (size_t)doc->node_count == start_node_count);
  return pos;
}

void hv_apply_node_attrs(struct html_node *node, const char *attrs,
                               size_t len) {
  char style_attr[256];
  char wh_buf[16];
  style_attr[0] = '\0';
  ...
  if (hv_has_boolean_attr(attrs, len, "open")) node->open = 1;
}
