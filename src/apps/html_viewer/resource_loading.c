...
void html_viewer_request_sync(struct html_viewer_app *app,
  ...
  if (hv_strncmp(url, "about:home", 10) == 0 ||
      ...
      hv_strncmp(url, "about:network", 13) == 0 ||
      hv_strncmp(url, "about:memory", 12) == 0 ||
...
int hv_doc_queue_pending_css(struct html_document *doc,
                                    const char *url) {
  if (!doc || !url || !url[0]) return 0;
