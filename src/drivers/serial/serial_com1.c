/* serial_com1.c: COM1 serial UART driver for debug output and input fallback.
 * Hyper-V Gen 2 has no PS/2 so serial is the primary fallback input.
 * Originally drivers/serial/com1.c; renamed to dodge the reserved DOS
 * device name "COM1" that mangled the file on macOS exFAT volumes. */
